...
extension DatabaseManager
{
    ...
    /// Gets all messages for gives conversation
    public func getAllMessagesForConversations(with id: String, completion: @escaping (Result<[Message], Error>) -> Void)
    {
        database.child("\(id)/messages").observe(.value, with: { snapshot in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                {
                    // video
                    guard let videoUrl = URL(string: content), let placeholder = UIImage(systemName: "video.bubble.left") else {
                        return nil
                    }
...
struct ChatAppUser
{
    ...
    {
        //tech030893-gmail-com_profile_picture.png
